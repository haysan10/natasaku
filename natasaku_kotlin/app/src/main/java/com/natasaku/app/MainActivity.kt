    ...
    private fun handleIntent(intent: Intent?) {
        ...
        if (intent?.action == Intent.ACTION_VIEW && uri != null) {
            ...
            if (::webView.isInitialized) {
                when (host) {
                    "jajan" -> evaluateJavascriptSafely("window.openTransactionModal && window.openTransactionModal('expense');")
                    "pemasukan" -> evaluateJavascriptSafely("window.openTransactionModal && window.openTransactionModal('income');")
                    "tabung", "target-tabungan" -> evaluateJavascriptSafely("window.changeTab && window.changeTab('savings');")
                    "budget" -> evaluateJavascriptSafely("window.openBudgetModal && window.openBudgetModal();")
                    "laporan" -> evaluateJavascriptSafely("window.changeTab && window.changeTab('reports');")
                    "riwayat" -> evaluateJavascriptSafely("window.changeTab && window.changeTab('transactions');")
                    "detail-jatah" -> evaluateJavascriptSafely("window.openDailyDetailModal && window.openDailyDetailModal();")
                    "home" -> evaluateJavascriptSafely("window.changeTab && window.changeTab('home');")
                }
            } else {
                // If webView is not ready yet, store the action and apply onPageFinished
