...
interface ExpenseTransactionDao {
    ...
    fun observeFiltered(periodId: String, query: String, category: String?): Flow<List<ExpenseTransactionEntity>>
    ...
    fun pagingByPeriod(periodId: String): PagingSource<Int, ExpenseTransactionEntity>
    ...
    fun pagingByPeriodFiltered(
        periodId: String,
        query: String,
        category: String?,
    ): PagingSource<Int, ExpenseTransactionEntity>

    @Query("SELECT * FROM expense_transaction WHERE id = :id LIMIT 1")
    suspend fun findById(id: String): ExpenseTransactionEntity?

    @Query("UPDATE expense_transaction SET deletedAt = :deletedAt WHERE id = :id")
    suspend fun softDelete(id: String, deletedAt: Instant)

    @Query("DELETE FROM expense_transaction WHERE amount <= 0")
    suspend fun deleteCorruptedZeroAmount()
}
